...
export const DUE_DATE_TOKEN_RE = /^(?:📅|#?due:|@)(\d{4}-\d{2}-\d{2})$/i;

const DUE_DATE_STRIP_RE = /\s*(?:📅|#?due:|@)(\d{4}-\d{2}-\d{2})\b/gi;

export function extractDueDate(text) {
  const match = DUE_DATE_RE.exec(String(text ?? ""));
  return match ? match[1] : null;
}

export function extractDueDateToken(text) {
  const match = DUE_DATE_TOKEN_RE.exec(String(text ?? "").trim());
  return match ? match[1] : null;
}

export function stripDueDateTokens(text) {
  return String(text ?? "")
